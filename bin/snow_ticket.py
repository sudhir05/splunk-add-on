...
class SnowTicket(object):
    ...
    def _handle_event(self, event):
        ...
        for _ in range(3):
            ok, result = self._do_event(http, endpoint, event_data, headers, retry=_)
            if ok:
                return result
        return self._get_failure_message()

    def _do_event(self, http, endpoint, event_data, headers, retry=0):
    ...
    def _get_service_now_account(self):
        ...
        try:
            ...
            self.logger.info("Getting details for account '{}'".format(self.account))
            ...
            # Get account details
                ...
                snow_account["account"] = self.account
                ...
                snow_account["disable_ssl_certificate_validation"] = account_details.get("disable_ssl_certificate_validation", 0)

                account_auth_type = account_details.get("auth_type", "basic")

                if account_auth_type not in ["basic", "oauth"]:
                    si.generateErrorResults("'{}' is not configured with the desired authentication type. Expected values are 'basic' and 'oauth'. Current value is '{}'".format(self.account, account_auth_type))
                    raise Exception("'{}' is not configured with the desired authentication type. Expected values are 'basic' and 'oauth'. Current value is '{}'".format(
                        self.account, account_auth_type
                    ))
                ...
                for field in account_access_fields:
                    if field in account_details.keys():
                        if field in ["password"] and account_details.get("auth_type","basic") == "basic":
                            snow_account[field] = account_details[field].encode("ascii", "replace").decode("ascii")
                        elif field in [ "client_id", "client_secret", "access_token", "refresh_token"] and account_details.get("auth_type","basic") == "oauth":
                            snow_account[field] = account_details[field].encode("ascii", "replace").decode("ascii")
                        else:
    ...
    def _handle_response(self, response, content):
        if response.status in (200, 201):
            ...
            # If HTTP status = 401 and auth_type = oauth, there is a possibility that access token is expired
            elif response.status == 401 and self.snow_account["auth_type"] == "oauth":
                result = {"retry": False}
                self.logger.error("Failed to create ticket. Return code is {0} ({1}). Failure potentially caused by "
                ...
                if update_status:
                    ...
                    self.logger.error("Unable to regenerate new access token. Failure potentially caused by "
                                      "the expired refresh token. To fix the issue, reconfigure the account and try again.")

                    si.parseError("Failed to create ticket. Return code is {0} ({1}). Failed to generate a new access token to resolve the issue. "
                                  "Failure potentially caused by the expired refresh token. To fix the issue, reconfigure "
                                  "the account and try again.".format(response.status, response.reason))
