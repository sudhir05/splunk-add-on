...
class ThreadPool(object):
    ...
    def enqueue_funcs(self, funcs, block=True):
        ...
        if not self._started:
            _LOGGER.info("ThreadPool has already stopped.")
            return
    ...
    def _do_resize_according_to_loads(self):
        ...
        _LOGGER.debug("current_thr_size=%s, free_thrs=%s, work_size=%s",
    ...
    def _do_admin(self):
        ...
        while 1:
            ...
            if wakup is None:
                ...
                self._do_resize_according_to_loads()
        _LOGGER.info("ThreadPool admin thread=%s stopped.",
    ...
    def _run(self):
        ...
        _LOGGER.debug("Worker thread %s stopped.",
                      threading.current_thread().getName())
