...
def build_http_connection(config, timeout=120):
    ...
    # Check if proxy_type is present in proxy_type_to_code
    ...
    # if proxy_type is None assign default value to it
        ...
        _LOGGER.warn("Value of 'proxy_type' parameter missing. Using default value='http' to continue data collection.")
    # Exception if proxy_type has unexpected value
    else:
        raise Exception("Got unexpected value {} of proxy_type parameter. Supported values of proxy_type parameter are "
                        "http, http_no_tunnel, socks4, socks5".format(config.get('proxy_type')))
    ...
    # Verifying if proxy is enabled for data collection
    if utils.is_true(config.get("proxy_enabled")):
    ...
    # Username and password will be added only if authentication type is 'basic'
